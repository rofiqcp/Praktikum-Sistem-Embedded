...
static void i2c_raw_write_read(uint8_t dev_addr, uint8_t reg_addr,
                                uint8_t *read_buf, size_t read_len)
{
    ...
    i2c_cmd_handle_t cmd = i2c_cmd_link_create();
    if (cmd == NULL) {
        ESP_LOGE(TAG, "Gagal membuat I2C command link!");
        return;
    }
    ...
    uint8_t addr_write = (dev_addr << 1) | I2C_MASTER_WRITE;
    ...
    i2c_master_write_byte(cmd, addr_write, true); /* true = cek ACK */
...
static void i2c_raw_write_only(uint8_t dev_addr, uint8_t reg_addr,
                                uint8_t *write_data, size_t write_len)
{
    esp_err_t err;
    ...
    uint8_t addr_w = (dev_addr << 1) | I2C_MASTER_WRITE;
    ESP_LOGI(TAG, "[2] ADDR+W: 0x%02X -> menunggu ACK", addr_w);
    i2c_master_write_byte(cmd, addr_w, true);
    ...
    for (size_t i = 0; i < write_len; i++) {
        ...
        i2c_master_write_byte(cmd, write_data[i], true);
    }
...
static void i2c_raw_multi_read(uint8_t dev_addr, uint8_t start_reg, size_t count)
{
    ESP_LOGI(TAG, "");
    ...
    esp_err_t err = i2c_read_reg(dev_addr, start_reg, data, count);
...
static void raw_i2c_task(void *pvParameters)
{
    ...
    while (1) {
        ...
        i2c_raw_write_read(TARGET_ADDR, TARGET_REG, read_data, READ_LENGTH);

        /* 3. Demonstrasi multi-byte read */
        i2c_raw_multi_read(TARGET_ADDR, 0x88, 6);

        /* 4. Coba alamat alternatif */
        uint8_t alt_data[1] = {0};
        ESP_LOGI(TAG, "\n--- Coba alamat alternatif ---");
        i2c_raw_write_read(ALT_ADDR_1, 0x00, alt_data, 1);
