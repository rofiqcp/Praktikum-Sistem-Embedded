/**
 * STM32_02_UART_Interrupt_RX
 * --------------------------
 * USART1 interrupt-driven receive.
 * HAL_UART_Receive_IT receives one byte at a time.
 * HAL_UART_RxCpltCallback echoes the byte and re-arms the receive.
 * LED PC13 toggles on every received byte.
 */
...
int main(void)
{
    ...
    const char *banner = "UART Interrupt RX Ready\r\n";
    ...
    while (1)
    {
        /* Main loop is free – RX handled in callback */
        HAL_Delay(100);
    }
}
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
    if (huart->Instance == USART1)
    {
        ...
        HAL_UART_Transmit(&huart1, (uint8_t *)&rx_byte, 1, UART_TIMEOUT);
...
/* ================================================================
 *  USART1 IRQ Handler – must call HAL handler
 * ================================================================ */
...
/* ================================================================
 *  USART1 Init – PA9 TX, PA10 RX  + NVIC for interrupt
 * ================================================================ */
static void MX_USART1_UART_Init(void)
{
