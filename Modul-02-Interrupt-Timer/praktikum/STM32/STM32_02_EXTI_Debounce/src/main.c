/**
 * @file main.c
 * @brief STM32_02_EXTI_Debounce - EXTI Interrupt with Software Debounce
 *
 * External interrupt on PB0 (falling edge), toggle LED on PC13
 * ISR sets a flag, main loop checks flag + time elapsed >= DEBOUNCE_MS
 * This prevents multiple triggers from mechanical bounce
...
/* ---- Volatile flags shared between ISR and main loop ---- */
...
/* ============================================================
 *  EXTI Callback - set flag, record timestamp
 * ============================================================ */
...
int main(void)
{
    ...
    while (1)
    {
        if (exti_flag)
        {
            ...
            if ((now - last_exti_tick) >= DEBOUNCE_MS)
            {
                last_exti_tick = now;
